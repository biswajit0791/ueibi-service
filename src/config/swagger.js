...
const options = {
  definition: {
      ...
      schemas: {
        ...
        Task: {
          ...
          properties: {
            ...
            weight: { type: 'integer', example: 30, description: 'Percentage share of the parent goal. A goal’s tasks must total exactly 100% before any of them can be started.' },
            actualStartDate: { type: 'string', format: 'date-time', nullable: true, description: 'When work actually began, stamped the first time the task leaves ‘todo’. Null on tasks that predate this field.' },
            actualCompletionDate: { type: 'string', format: 'date-time', nullable: true, description: 'When work actually finished, stamped on ‘done’ and cleared if the task is reopened.' },
            timing: { type: 'object', nullable: true, description: 'Delay and overdue figures derived from the planned dates against the actual ones. Computed server-side so every screen reports the same number; fields are null rather than 0 when no actual date was recorded.', properties: {
              startDelayDays: { type: 'integer', nullable: true, description: 'Positive when work began later than planned.' },
              completionDelayDays: { type: 'integer', nullable: true, description: 'Positive when work finished after the due date.' },
              daysOverdue: { type: 'integer', nullable: true, description: 'Days past due while still unfinished.' },
              isStarted: { type: 'boolean' },
              isComplete: { type: 'boolean' },
              isOverdue: { type: 'boolean' },
              startedLate: { type: 'boolean' },
              finishedLate: { type: 'boolean' },
              state: { type: 'string', enum: ['NOT_STARTED', 'ON_TRACK', 'DUE_TODAY', 'OVERDUE', 'COMPLETED_ON_TIME', 'COMPLETED_LATE', 'COMPLETED'] },
            } },
    ...
    paths: {
      ...
      '/goals/{id}/weight-summary': {
        get: {
          tags: ['Goals'],
          summary: 'Task weight position and lock state for a goal',
          operationId: 'getGoalWeightSummary',
          description: "A goal's tasks must total exactly 100% before any of them can be worked on. This reports the total, what is left to allocate, whether execution is unlocked and, when it is not, a sentence explaining what has to change. The total is always summed from the database; no weight figure sent by a client is read or trusted. Goals whose work had already begun when weighting was introduced carry `exempt: true` and are never locked — applying the rule retroactively would have frozen live goals mid-cycle. Same access rule as reading the goal.",
          security: [{ userCookie: [] }],
          parameters: [
            { name: 'id', in: 'path', required: true, schema: { type: 'string' }, description: 'Goal ID' },
          ],
          responses: {
            200: {
              ...
              content: { 'application/json': { schema: { type: 'object', properties: {
                ...
                exempt: { type: 'boolean', description: 'Grandfathered: work had already begun before the rule existed.' },
                locked: { type: 'boolean', description: 'Execution is blocked until the total is exactly 100.' },
                readiness: { type: 'string', enum: ['READY', 'NOT_READY'], description: 'Derived, never stored. READY means the task structure is valid and work may begin — not that the goal is finished.' },
                workStarted: { type: 'boolean' },
                completedCount: { type: 'integer' },
                reason: { type: 'string', example: 'Task weights total 80%. Allocate the remaining 20% to unlock execution.' },
                tasks: { type: 'array', items: { type: 'object', properties: {
                  id: { type: 'string' }, title: { type: 'string' },
                  weight: { type: 'integer' }, status: { type: 'string' }, employeeId: { type: 'string' },
                } } },
              } } } },
            },
            401: { description: 'Unauthorized' },
            403: { description: 'Access forbidden: you do not have permission to view this goal' },
            404: { description: 'Goal not found' },
          },
        },
      },
